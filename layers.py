...
class AHAG(object):
    def __init__(
        ...
        with tf.variable_scope("self_user_attention"):
            ...
            user_self_feature = tf.multiply(self.self_attn_user_expanded, user_feature_02) #U
        ...
        with tf.variable_scope("trilinear_matrix"):
            ...
            user_pooling = user_self_feature*left_attention_01 #tku
            item_pooling = item_self_feature*right_attention_01 #tji
        ...
        with tf.variable_scope("get_fea", reuse=tf.AUTO_REUSE):
            ...
            self.u_feas_review = tf.nn.tanh(tf.matmul(self.h_drop_u, Wuu) + bu)  # formulate 14
            ...
            self.i_feas_review = tf.nn.tanh(tf.matmul(self.h_drop_i, Wii) + bi)   # formulate 14

        with tf.name_scope("gating_network"):

            #only fusion gate
            self.u_feas = fusion_gating_net(self.u_feas_review, self.uid, n_latent) # Uu
            self.item_first_filter = fusion_gating_net(self.i_feas_review, self.iid,n_latent) # Vi


            self.i_feas, self.item_fea_instance = gating_network(self.u_feas, self.item_first_filter, n_latent) #ViF



        with tf.name_scope('fm'):
            self.FM = tf.concat([self.u_feas, self.i_feas], 1)  # Uu ViF
            self.z = self.FM
            ...
            self.predictions = one + inter_last + b  # 22
