...
def optimized_trilinear_for_attention(args, user_len, item_len, input_keep_prob=1.0,
    ...
    arg0_shape = args[0].get_shape().as_list() # U
    arg1_shape = args[1].get_shape().as_list() # V
    ...
    with tf.variable_scope(scope):
        ...
        res =  subres0 + subres1 +subres2  # formulat 10
...
def fusion_gating_net(u_feature, uid_feature, n_latent, scope="fusion_gating_net"):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # feature gating
        feature_rate = tf.sigmoid(tf.matmul((tf.add(u_feature, uid_feature)),W_gating)+b_gate)  #formulate 16
        fusion_fea = tf.multiply((1 - feature_rate), uid_feature) + tf.multiply(feature_rate, u_feature)
        fuse_feature = tf.sigmoid(tf.matmul(uid_feature,W_gate)+tf.matmul(fusion_fea,W_vgate)) #formulate 17
...
def gating_network(u_feas, i_feas,n_latent, scope="gating_network"):
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        # feature gating
        feature_gate = tf.sigmoid(tf.matmul(u_feas, W_ugate) + tf.matmul(i_feas, W_igate) + bi_gate) #formulate 18
